...
def update_monthly_bill(bill_id):
    ...
    try:
        ...
        cursor.execute(
            ...
            (data.get('StudentID'), data.get('BillingMonth'), data.get('BillingYear'), data.get('RoomRentCharges'), data.get('MessCharges'), data.get('OtherCharges'), data.get('TotalAmount'), data.get('DueDate'), data.get('PaymentStatus'), bill_id)
        )
        conn.commit()
